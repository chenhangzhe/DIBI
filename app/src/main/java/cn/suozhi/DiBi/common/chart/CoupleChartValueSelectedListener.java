...
/**
 * 图表联动高亮监听
 * 使用时注意 src 和 dst 的位置，正确配置 {@link #onValueSelected(Entry, Highlight)} 中y值
 */
        ...
        if (dstCharts != null) {
            for (BarLineChartBase chart : dstCharts) {
                ...
                if (chart instanceof CombinedChart) {
                    CombinedChart.DrawOrder[] draw = ((CombinedChart) chart).getDrawOrder();
                    if (draw != null && draw[0] == CombinedChart.DrawOrder.BAR) {
