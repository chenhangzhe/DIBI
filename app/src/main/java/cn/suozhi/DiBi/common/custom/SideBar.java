...
/**
 * SideBar类就是ListView右侧的字母索引View，
 * 我们需要使用setTextView(TextView mTextDialog)来设置用来显示当前按下的字母的TextView,
...
public class SideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < b.length; i++) {
            paint.setColor(ContextCompat.getColor(getContext(), R.color.bk2C));
                ...
                if (oldChoose != c) {
                    if (c >= 0 && c < b.length) {
                        if (onTouchingListener != null) {
                            onTouchingListener.onTouchingLetterChanged(b[c]);
                        }
                        if (mTextDialog != null) {
