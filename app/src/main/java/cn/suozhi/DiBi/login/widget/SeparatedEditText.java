package cn.suozhi.DiBi.login.widget;
...
public class SeparatedEditText extends AppCompatEditText {
    ...
    private RectF boxReSolidRctF;//小方块、小矩形
    ...
    public SeparatedEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        borderColor = ta.getColor(R.styleable.SeparatedEditText_borderColor, ContextCompat.getColor(getContext(), R.color.redE0));
        blockColor = ta.getColor(R.styleable.SeparatedEditText_blockColor, ContextCompat.getColor(getContext(), R.color.color_f8f9fd));
        textColor = ta.getColor(R.styleable.SeparatedEditText_text_Color, ContextCompat.getColor(getContext(), R.color.text_color_dark));
        cursorColor = ta.getColor(R.styleable.SeparatedEditText_cursorColor, ContextCompat.getColor(getContext(), R.color.text_color_dark));
        ...
        cursorWidth = (int) ta.getDimension(R.styleable.SeparatedEditText_cursorWidth, dip2px(context, 1));
        borderWidth = (int) ta.getDimension(R.styleable.SeparatedEditText_borderWidth, dip2px(context, 1));
    ...
    private void drawRect(Canvas canvas) {
        for (int i = 0; i < maxLength; i++) {
            ...
            boxRectF.set(spacing * (i + 1) + boxWidth * i, 0,
                    ...
                    boxHeight);
            //上+1 下 -1 处理 验证码边框粗细不一致的问题
            boxReSolidRctF.set(spacing * (i + 1) + boxWidth * i, 1,
                    spacing * (i + 1) + boxWidth * i + boxWidth,
                    boxHeight-1);

            if (type == TYPE_SOLID) {
                canvas.drawRoundRect(boxRectF, corner, corner, blockPaint);
                canvas.drawRoundRect(boxReSolidRctF, corner, corner, borderPaint);
            } else if (type == TYPE_UNDERLINE) {
                if (i <= currentCursorIndex) {
                    //光标所在位置之前的输入过的数字下划线颜色
                    borderPaint.setColor(borderColor);
                } else {
                    //光标所在位置之后的没有输入过数字下划线的颜色
                    borderPaint.setColor(getResources().getColor(R.color.redE0));
                }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        contentText = text;
        currentCursorIndex = text.length()-1;
        invalidate();
