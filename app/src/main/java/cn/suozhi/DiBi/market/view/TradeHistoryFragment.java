...
import cn.suozhi.DiBi.market.adapter.TradeHistoryRVAdapter;
import cn.suozhi.DiBi.market.model.OrderEntity;

/**
 * 币币-历史委托
 */
public class TradeHistoryFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
        RadioGroup.OnCheckedChangeListener, OnCallbackListener,
    ...
    private String type;
    private TradeHistoryRVAdapter recyclerAdapter;
    ...
    protected void init(View v) {
        ...
        recyclerAdapter = new TradeHistoryRVAdapter(getActivity(), this);
    ...
    private void getData() {
        if (loadListener != null && recyclerAdapter != null) {
            if (SharedUtil.isLogin(getActivity())) {
                ...
                loadListener.onLoad(Constant.Code.Type_History_Entrust, Constant.Code.Trade_History_Entrust,
    ...
    public void updateOrder(Messages.DmOrder order) {
        if (order == null) {
            return;
        }
        int index = getIndex(order.getOrderId());
        if (index >= 0) {
            dataList.remove(index);
            Symbol symbol = loadListener == null ? null : loadListener.getSymbol();
            dataList.add(index, AppUtil.getOrder(order, symbol));
            recyclerAdapter.notifyItemChanged(index);
        }
    }

    private int getIndex(long orderId) {
        int index = -1;
        if (dataList == null) {
            return index;
        }
        for (int i = 0; i < dataList.size(); i++) {
            if (dataList.get(i).getOrderId() == orderId) {
                index = i;
                break;
            }
        }
