...
public abstract class BaseMultiItemFetchLoadAdapter<T, K extends BaseViewHolder> extends BaseFetchLoadAdapter<T, K> {
    ...
    protected void convert(final K baseHolder, final T item, final int position, boolean isScrolling) {
        ...
        if (h instanceof MsgViewHolderMyOrder) {

            RecyclerViewHolder finalH = h;
            ((MsgViewHolderMyOrder) h).setOnShowOrderInfoLisenter(new MsgViewHolderMyOrder.OnOrderNoClikLisenter() {
                @Override
                public void onOrderClick(TextView tv_order_content, String title, TextView tv_order_title, String orderNo, TextView tvDetail, MsgDirectionEnum msgDirectionEnum) {
                    if (lisenter != null) {
                        if (msgDirectionEnum == MsgDirectionEnum.Out) {

                        } else {
                            lisenter.onOrderClick(tv_order_content, title, tv_order_title, orderNo, tvDetail);

                        }
                    }

                    if (msgDirectionEnum == MsgDirectionEnum.Out) {
                        ((MsgViewHolderMyOrder) finalH).view.setVisibility(View.GONE);
                    } else {
                        ((MsgViewHolderMyOrder) finalH).view.setVisibility(View.VISIBLE);
                    }
                }
    ...
    public interface OnOrderShowDataLisenter {
        void onOrderClick(TextView tv_order_content, String title, TextView tv_order_title, String orderNo, TextView tvView);
    }

    private OnOrderShowDataLisenter lisenter;


    public void setOnOrderShowDataLisenter(OnOrderShowDataLisenter lisenter) {
