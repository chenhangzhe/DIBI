package com.netease.nim.uikit.business.module.list;
...
import com.netease.nim.uikit.business.module.Container;
...
public class MessageListPanelEx {
    ...
    private void initListView(IMMessage anchor) {
        ...
        adapter.setOnOrderShowDataLisenter(new BaseMultiItemFetchLoadAdapter.OnOrderShowDataLisenter() {
            @Override
            public void onOrderClick(TextView tv_order_content, String title, TextView tv_order_title, String orderNo, TextView tvDetail) {
                if (lisenter2 != null) {
                    lisenter2.onOrderClick(tv_order_content, title, tv_order_title, orderNo, tvDetail);
                }
            }
        });
    }

    public interface OnShowOrderInfoLisenter {
        void onOrderClick(TextView tv_order_content, String title, TextView tv_order_title, String orderNo, TextView tvDetail);
    }

    private OnShowOrderInfoLisenter lisenter2;


    public void setOnShowOrderInfoLisenter(OnShowOrderInfoLisenter lisenter) {
        this.lisenter2 = lisenter;
    }


    public interface OnOrderNoClikLisenter {
        void onOrderClick(String no, String orderNo);
    }

    private OnOrderNoClikLisenter lisenters;


    public void setOnOrderNoClik(OnOrderNoClikLisenter lisenter) {
        this.lisenters = lisenter;
    }
    ...
    private OnItemClickListener listener = new OnItemClickListener() {
        ...
        public void onItemClick(IRecyclerView adapter, View view, int position) {
            IMMessage item = MessageListPanelEx.this.adapter.getItem(position);
            //处理自定义消息的点击事件
            if (item.getMsgType() == MsgTypeEnum.custom) {
                JSONObject object = JSON.parseObject(item.getAttachment().toJson(true));
                int type = object.getInteger("type");
                String dataStr = object.getString("data");
                if (type == CustomAttachmentType.MyOrderCustomMsg) {
                    if (!TextUtils.isEmpty(dataStr)) {
                        JSONObject object2 = JSON.parseObject(dataStr);
                        String status = object2.getString("msg");
                        String orderNo = object2.getString("orderNo");
                        if (!TextUtils.isEmpty(orderNo)) {
                            if (lisenters != null) {
                                lisenters.onOrderClick(orderNo, status);
                            }
                        }
                    }
                }
    ...
    public void onMsgSend(IMMessage message) {
        if (!TextUtils.isEmpty(container.account) && !container.account.equals(message.getSessionId())) {
            return;
        }
            ...
            public void onResult(int code, List<IMMessage> messages, Throwable exception) {
                ...
                if (messages != null) {
                    for (int i = 0; i < messages.size(); i++) {
                        if (messages.get(i).getDirect() == MsgDirectionEnum.Out && messages.get(i).getMsgType() == MsgTypeEnum.custom) {
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    private boolean receiveReceiptCheck(final IMMessage msg) {
        return msg != null
                ...
                && msg.getMsgType() != MsgTypeEnum.custom
                && msg.getMsgType() != MsgTypeEnum.text
                && msg.isRemoteRead();
